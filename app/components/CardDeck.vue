  ...
  <section class="glass interactive-surface motion-enter rounded-2xl p-4 sm:p-6">
    <header :class="deckVisible ? 'mb-5' : ''">
      ...
      <div class="mt-1 min-h-4 font-mono text-xs text-[var(--color-fg-dim)]">
          ...
          <span v-else-if="disabled">карты заблокированы</span>
    ...
    <Transition name="deck-collapse">
      <div v-if="deckVisible" class="grid grid-cols-3 gap-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-9">
        <button
          v-for="(card, index) in store.deck"
          :key="card"
          type="button"
          class="group relative rounded-2xl stagger-enter"
          :disabled="disabled"
        ...
        >
          <div class="flip-perspective aspect-[2/3] w-full">
            <div
              class="card-glow relative h-full w-full overflow-hidden rounded-2xl border border-[var(--color-border)] transition-all duration-300 ease-out"
              :class="[
                disabled
                  ? 'cursor-not-allowed opacity-50'
                  : 'cursor-pointer hover:-translate-y-2 hover:scale-[1.04] active:scale-[0.98]',
                selected === card ? 'card-glow-active border-[var(--color-accent-blue)]/60' : '',
              ]"
            ...
            >
              <div
                ...
                :class="[!disabled && 'group-hover:opacity-100', selected === card ? 'opacity-100' : '']"
              ...
              <div
                class="pointer-events-none absolute inset-x-0 bottom-0 h-1 opacity-0 transition-opacity duration-300"
                :class="[!disabled && 'group-hover:opacity-100', selected === card ? 'opacity-100' : '']"
